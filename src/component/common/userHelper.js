...
import {Input,Text,Button,Form,Item,Container,
  Content,Label,Picker,Icon} from 'native-base'
...
export class InputElement extends React.PureComponent<Props> {
  ...
  render () {
    ...
    return (<View style={{ flexDirection: 'row', }}>
      ...
      <View>
        <Input style={{...Styles.input}} value={renderEmptyStringIfUndefined(state[eachRow.stateName])} keyboardType={eachRow.keyboardType} onChangeText={text => updateState({ [eachRow.stateName]: text })}/>
      </View>
    </View>
    </View>)
  }
    ...
    return (<View style={{ flexDirection: 'row', }}>
      ...
      <View style={{...Styles.input}}>
        <Picker
            ...
            onValueChange={(item) => updateState({ [eachRow.stateName]: item })}
          >
            {dataList.map((eachItem) => {
              return <Picker.Item key={eachItem.key} label={eachItem.label} value={eachItem.value} />
            })}
            </Picker>
      </View>
    </View>
...
export class CustomButton extends React.PureComponent<Props> {
  ...
  render () {
    ...
    return <View key={eachRow.displayName} style={{ flexDirection: 'row', marginTop: 10 }}>
      <View style={{ ...Styles.flex1 }}>
        <Button onPress={() => Oncancel()} {...eachRow.leftButton.buttonStyles}>
          <Text>{eachRow.leftButton.displayName}</Text>
        </Button>
      </View>
      <View style={{ ...Styles.flex1 }}>
        <Button onPress={() => onClick(eachRow.rightButton.displayName)} {...eachRow.rightButton.buttonStyles}>
